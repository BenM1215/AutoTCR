...
runID <- '' # enter a run name between the '' i.e. penta5run5

# Email address to send the run report to
emailAddress <- 'ucbpmar@ucl.ac.uk'
sendEmail <- F # TRUE or FALSE. If you are getting Java errors, then set to FALSE.

# Contains all data files
inputPath <- '~/Desktop/PENTA11Run5Repeat/Input'

# Path to write output to
outputPath <- '~/Desktop/PENTA11Run5Repeat/Output'

# Contains all decombinator scripts and this .Rmd script
scriptPath <- '~/Desktop/PENTA11Run5Repeat/Scripts'

# Contains a python installation with all prerequisite decombinator packages installed
python <- '~/anaconda2/bin/python' # if you are unsure of what to put here, type 'which python' in Terminal and enter the result

# alpha and beta, or just 1?
numberChains <- 2

# Set to TRUE to run collapsinator with -N, allowing for unknown base calls, otherwise, set to false.
collapsinatorN <- T # TRUE or FALSE

# Set to TRUE the first time AutoTCR is run on the computer to install the necessary packages
installAutoTCR <- F # TRUE or FALSE
```

# Installs packages for AutoTCR
Only necessary to run once, if not run before

```{r}
packages <- c('data.table', 'zoo', 'scales', 'ggplot2', 'ggthemes', 'dplyr', 'plotly', 
              'pracma', 'Hmisc', 'rpart', 'survminer', 'knitr', 'mailR', 'grid', 
              'stringr', 'ggrepel', 'ggjoy', 'RColorBrewer', 'stringdist', 'ape', 
              'dendextend', 'reshape2', 'gplots', 'ineq', 'vegan')
...
if (length(indexFileName) == 0){
  print (paste('index file not found in', inputPath))
  print('Please check for errors in your file structure and/or file naming.')
  print('The script will now exit with an error.')
}
...
tra <- read.csv(paste(scriptPath,'/traAntigen.csv', sep=''), header = T, stringsAsFactors = F, sep = '\t')
trb <- read.csv(paste(scriptPath,'/trbAntigen.csv', sep=''), header = T, stringsAsFactors = F, sep = '\t')
...
if (length(CDR3Files)>0){
  ...
  cdr3Dat$score[cdr3Dat$chain=='a'] <- tra$Score[match(cdr3Dat$V1[cdr3Dat$chain=='a'],tra$CDR3)]
  cdr3Dat$antigen[cdr3Dat$chain=='b'] <- trb$Epitope.species[match(cdr3Dat$V1[cdr3Dat$chain=='b'],trb$CDR3)]
  cdr3Dat$score[cdr3Dat$chain=='b'] <- trb$Score[match(cdr3Dat$V1[cdr3Dat$chain=='b'],trb$CDR3)]
...
# CDR3 Density Plots - unique clones

```{r, warning=FALSE}
# # # #  CDR3 Density Plots - frequency
CDR3FreqDensOutputPath <- paste(plotsOutputPath,'/CDR3UniqueFrequencyDensity',sep='')
...
for (i in unique(cdr3Dat$sampleID)){
  ...
  ggp <- ggplot(data=cdr3DatTemp)+
    ...
    scale_x_log10()+
    theme_classic()+
    labs(title=paste(i, '\t\t Num. Reads =', sum(cdr3DatTemp$V2)), x='Freq. of Unique CDR3 Sequences', y='Density')
  ...
  for (j in unique(cdr3Dat$month[cdr3Dat$ID==i])){
    for (k in unique(cdr3Dat$chain[cdr3Dat$ID==i & cdr3Dat$month == j])){
      ...
      ggp <- ggplot(data=cdr3DatTemp)+
        ...
        # scale_x_log10()+
        theme_classic()+
        labs(title=paste(i, j, k, paste('\t Num. Reads = ', sum(cdr3DatTemp$V2), sep = ''), sep = ''), x='Freq. of Unique CDR3 Sequences', y='Density')
...
#  CDR3 Frequency Density (Expanded)

```{r}
# # # #  CDR3 Density Plots - frequency
CDR3FreqDensOutputPath2 <- paste(plotsOutputPath,'/CDR3FrequencyDensity',sep='')
dir.create(CDR3FreqDensOutputPath2)
IndividualCDR3FreqDensOutputPath2 <- paste(CDR3FreqDensOutputPath2,'/IndividualPlots',sep='')
dir.create(IndividualCDR3FreqDensOutputPath2)
...
for (i in unique(cdr3Dat$sampleID)){
  ...
  ggsave(paste(IndividualCDR3FreqDensOutputPath2, '/', i, '.pdf', sep = ''), 
         ggp, width = 10, height = 10, units = c('cm'))
}
...
for (i in unique(cdr3Dat$ID)){
  ...
  pdf(paste(CDR3FreqDensOutputPath2, '/', i, 'SampleCDR3Frequency', '.pdf', sep = ''), width = 12, height = 8)
  multiplot(plotlist = ggps, cols=counter/2)
  dev.off()
}
...
for (i in unique(gnDat.expanded$ID)){
  for (j in unique(gnDat.expanded$chain[gnDat.expanded$ID==i])){
...
# Antigen Specificity
```{r}
specificityOutputPath <- paste(plotsOutputPath,'/AntigenSpecificity',sep='')
dir.create(specificityOutputPath)

for (i in unique(cdr3Dat$ID)){
  
  antigenOutputPath <- paste(specificityOutputPath, '/', i, 'Antigens', sep='')
  dir.create(antigenOutputPath)
  
  temp <- na.omit(cdr3Dat.expanded[cdr3Dat.expanded$ID==i,])
  temp <- unique(temp)
  if (length(temp$ID) > 0){
    temptemp <- aggregate(V2 ~ antigen + month + score, temp, sum)
    temp <- aggregate(V2 ~ antigen + month, temp, sum)
    
    # normalise temp$V2 as % of total
    temp$norm <- NA
    for (k in unique(temp$month)){
      numReads <- sum(cdr3Dat$V2[cdr3Dat$ID==i & cdr3Dat$month==k])
      temp$norm[temp$month==k] <- (100/numReads)*temp$V2[temp$month==k]
    }
    
    ggp <- ggplot(data = temp)+
      geom_point(aes(x = month, y=norm, group=antigen, colour=antigen))+
      geom_line(aes(x = month, y=norm, group=antigen, colour=antigen))+
      labs(x='Month', y='% of Total Reads', colour='Antigen Match', title=i)+
      theme_classic()
    
    ggsave(paste(specificityOutputPath, '/', i, '.pdf', sep = ''), 
           ggp, width = 9, height = 7, units = c('cm'))
    
    for (j in unique(temptemp$antigen)){
      
      temptemp$norm <- NA
      for (k in unique(temptemp$month)){
        numReads <- sum(cdr3Dat$V2[cdr3Dat$ID==i & cdr3Dat$month==k])
        temptemp$norm[temptemp$month==k] <- (100/numReads)*temptemp$V2[temptemp$month==k]
      }
      temptemptemp <- temptemp[temptemp$antigen==j,]
      
      ggp <- ggplot(data = temptemptemp)+
        geom_point(aes(x = month, y=norm, group=as.factor(score), colour=as.factor(score)))+
        geom_line(aes(x = month, y=norm, group=as.factor(score), colour=as.factor(score)))+
        labs(x='Month', y='% of Total Reads', colour='Evidence Score', title = paste(i, '-', j))+
        theme_classic()
      
      ggsave(paste(antigenOutputPath, '/', j, '.pdf', sep = ''), 
             ggp, width = 9, height = 7, units = c('cm'))
    }
  }
}
